...
/// Every `#[command]` in src/main.rs. Declaring them in the app manifest makes
/// Tauri generate `allow-<command>`/`deny-<command>` permissions (underscores
/// become dashes) under permissions/autogenerated/ and, more importantly, makes
/// the IPC gate refuse any app command that no capability grants. Keep this
/// list, `generate_handler!` in main.rs and capabilities/default.json in sync.
const COMMANDS: &[&str] = &[
    "start_server",
    "stop_server",
    "restart_server",
...
fn main() {
    ...
    // Rebuild when the autogenerated command permissions are removed or edited
    // by hand, so the ACL never drifts from COMMANDS.
