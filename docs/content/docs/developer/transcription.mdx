...
Files longer than Whisper's 30-second window are transcribed window by window. On Apple Silicon mlx-audio does this internally; on the PyTorch path `backend/backends/pytorch_backend.py` switches to transformers' long-form generation (`truncation=False`, `return_timestamps=True`) for audio over 30 seconds, while shorter clips use the padded single window.
...
model: base          # optional: base, small, medium, large, turbo (default: the loaded size, initially "base")
```
