...
claude mcp add voicebox \
  ...
  --header "Authorization: Bearer $VOICEBOX_API_KEY" \
  --header "X-Voicebox-Client-Id: claude-code"
```

Every request needs an API key. On the machine that runs the desktop app,
**Settings → MCP → Copy API key** gives you the app's admin key; for a
headless server, create a client key with `python -m backend.keys create --id claude-code --role client`
(see [API Keys](/overview/api-keys)). A checked-in `.mcp.json` can reference the
key as `${VOICEBOX_API_KEY}` so the secret stays in your environment.
...
`Authorization` carries the API key and is the credential. Change `cursor`
to whatever name you want the binding to show up as in Voicebox → Settings →
MCP; that value is just an identifier for the per-client voice binding, not
a secret.
  ...
  ```json
  {
    "mcpServers": {
      "voicebox": {
        ...
        "env": { "VOICEBOX_CLIENT_ID": "claude-desktop", "VOICEBOX_API_KEY": "vbx_..." }
      }
    }
  }
  ```
...
for the shim to connect. `VOICEBOX_API_KEY` is optional on the machine that
runs the desktop app: the shim finds the app's own key file automatically
(set `VOICEBOX_API_KEY_FILE` to point it elsewhere).
...
- **Every call needs an API key.** `/mcp` is covered by the same bearer
  authentication as the REST API; an agent without a key gets `401`. Give
  agents on other machines a `client` key rather than the admin key, and
  send it only over loopback or HTTPS. See [API Keys](/overview/api-keys).
- **Client keys get the speaking tools only.** `voicebox.speak`,
  `voicebox.transcribe` (base64 audio) and `voicebox.list_profiles` work
  with a client key; `voicebox.list_captures` returns your dictation
  transcripts and needs an admin key. Tool calls count against the key's
  inference rate limit.
- **`audio_path` reads need an admin key on loopback.** Remote callers and
  client keys must send `audio_base64`, so a server on a shared network can
  never be used to read local files.
