...
`POST /generate/stream` sends audio sentence by sentence while the rest is still being synthesized, for apps that need a fast first response. See [Streaming](/developer/tts-generation#streaming) in the developer docs.
