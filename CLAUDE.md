...
| What GitHub CI runs | `ci.yml`: `bun run ci` (typecheck + `build:web`) plus a backend job (ruff on the paths in `backend/ruff-clean-paths.txt`, the torch-free tests listed in `backend/tests/torch-free.txt`, installed from `backend/requirements-ci.txt`). `docker.yml`: builds the CPU image, runs `scripts/docker-smoke.sh`, publishes to ghcr.io on `main` and `v*` tags (CUDA image on tags only). Biome and the torch-dependent tests never run in CI. |
| Regenerate the Docker dependency lock (`backend/requirements.lock`, needs `uv`) | `just lock-backend` after editing `backend/requirements.txt` or `backend/requirements-docker.in` |
| Download models ahead of time (image bake, init container) | `backend/venv/bin/python -m backend.preload kokoro whisper-turbo` (`--list` prints the names) |
| Smoke-test a built image (boot, Kokoro preload, client-key stream, graceful stop) | `scripts/docker-smoke.sh voicebox:local` |
...
| Docker (NVIDIA) | `docker compose -f docker-compose.yml -f docker-compose.cuda.yml up --build` (`PYTORCH_VARIANT=cu130` for driver 580+) |
| Docker (AMD ROCm) | `docker compose -f docker-compose.yml -f docker-compose.rocm.yml up --build` |
| Docker with TLS (Caddy) | add `-f docker-compose.tls.yml` with `VOICEBOX_DOMAIN` set |
...
- Env vars the backend actually reads: `VOICEBOX_API_KEY`, `VOICEBOX_API_KEY_FILE`, `VOICEBOX_API_KEYS_JSON`, `VOICEBOX_DISABLE_DOCS`, `VOICEBOX_RATE_LIMITING`, `VOICEBOX_MAX_QUEUE_DEPTH`, `VOICEBOX_MAX_BODY_MB`, `VOICEBOX_MEDIA_TOKEN_TTL` (all in `auth/settings.py`), `VOICEBOX_DATA_DIR` (`config.py`; `--data-dir` wins), `VOICEBOX_LOG_LEVEL` or `LOG_LEVEL` (`app.py`), `VOICEBOX_PRELOAD_MODELS` (`services/preload.py`), `VOICEBOX_RETENTION_DAYS` (`services/retention.py`), `VOICEBOX_DRAIN_TIMEOUT_S` (`lifecycle.py`), `VOICEBOX_MODELS_DIR` (sets `HF_HUB_CACHE`), `VOICEBOX_CORS_ORIGINS` (comma-separated extras), `VOICEBOX_OFFLINE_PATCH=0` (disables `utils/hf_offline_patch.py`), `VOICEBOX_BACKEND_VARIANT`, `VOICEBOX_CLOUD_URL` / `VOICEBOX_CLOUD_API_URL`, plus `HF_HUB_OFFLINE`, `HSA_OVERRIDE_GFX_VERSION`, `MIOPEN_*`, and uvicorn's `FORWARDED_ALLOW_IPS`. `VOICEBOX_FORCE_CPU` appears in old docs but nothing reads it.
- Server lifecycle (`backend/lifecycle.py`): the first SIGTERM/SIGINT sets a process-wide draining flag (hooked around uvicorn's own handlers at lifespan startup), after which `task_queue.ensure_capacity` raises `QueueFullError("draining")` (routes answer 503 + `Retry-After`), `GET /health/ready` answers 503 and the SSE loops end; `_run_shutdown` then calls `task_queue.shutdown()` to wait `VOICEBOX_DRAIN_TIMEOUT_S` for pending jobs before unloading models. Readiness also tracks `VOICEBOX_PRELOAD_MODELS`: `services/preload.py` loads each one as a `preload-*` job on the generation queue (so it never double-loads with a generation) and retries failures every two minutes. `services/retention.py` prunes daily when `VOICEBOX_RETENTION_DAYS` is set; `POST /maintenance/prune` runs it on demand.
- Docker: both image stages are `python:3.12-slim` (the backend uses 3.12-only syntax such as PEP 695 generics); `PYTORCH_VARIANT` (`cpu`, `cu128`, any `cu*`, `rocm`) picks the torch index and default version in the `Dockerfile`, everything else installs from `backend/requirements.lock` (uv-generated, torch and its CUDA runtime deliberately absent, `chatterbox-tts`/`hume-tada` pinned `--no-deps`). `.dockerignore` must keep `backend/venv` out of the build context.
- SQLite runs with `journal_mode=WAL`, `synchronous=NORMAL` and `busy_timeout=5000` (set per connection in `database/session.py`); backups must copy the `-wal`/`-shm` files or use `sqlite3 .backup`.
- New Python modules and tests go on the lists in `backend/ruff-clean-paths.txt` (ruff in CI) and, when they import no torch/librosa/pedalboard-heavy module, `backend/tests/torch-free.txt` (pytest in CI). `tests/test_progress.py` imports `utils.progress` as a top-level package, so `backend/utils/progress.py` must use absolute imports.
...
- `docs/content/docs/developer/*.mdx`: `architecture`, `setup`, `building`, `tts-engines` (engine guide with PyInstaller pitfalls), `model-management`, `tts-generation`, `transcription`, `voice-profiles`, `effects-pipeline`; `docs/content/docs/overview/deployment.mdx` is the operator checklist (variants, probes, warm-up, drain, retention, backups).
